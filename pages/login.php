...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Regular login process
        ...
        if(empty($username_err) && empty($password_err)) {
            ...
            $sql = "SELECT id, username, password, user_type FROM users WHERE BINARY username = ?";
            ...
            if($stmt = mysqli_prepare($conn, $sql)) {
                ...
                if(mysqli_stmt_execute($stmt)) {
                    ...
                    if(mysqli_stmt_num_rows($stmt) == 1) {
                        mysqli_stmt_bind_result($stmt, $id, $db_username, $hashed_password, $user_type);
                        if(mysqli_stmt_fetch($stmt)) {
                            ...
                            if($hashed_password === null || empty($hashed_password)) {
                                ...
                                // Additional case-sensitive check in PHP for extra security
                                if($username === $db_username && password_verify($password, $hashed_password)) {
                                ...
                                $login_err = "Username case mismatch. Please check your username spelling and case.";
                            } else {
                                // Password is not valid - continue to check if it might be a reset code
                                $password_login_success = false;
                            }
                            } // End else block for accounts with passwords
                        }
                    } else {
