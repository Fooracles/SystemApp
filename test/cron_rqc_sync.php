...
function ensureLogDirectory($dir) {
    if (!is_dir($dir)) {
        ...
        if (!mkdir($dir, 0755, true)) {
            // If directory creation fails, log to a fallback location
...
function logRqcSync($message) {
    ...
    if (!ensureLogDirectory($log_dir)) {
        // Fallback: just output to console if directory creation fails
        ...
        try {
            ...
            if (!empty($sheet_timestamp)) {
                ...
                if ($parsed_timestamp !== false) {
                    ...
                    // Try alternative formats - including Google Sheets format: dd/mm/yy hh:mm AM/PM
                    $timestamp_formats = [
                        'd/m/y h:i A',        // 09/03/25 04:31 PM (Google Sheets format)
                        'd/m/y h:i:s A',      // 09/03/25 04:31:00 PM (with seconds)
                        'd/m/Y h:i A',        // 09/03/2025 04:31 PM (4-digit year)
                        'd/m/Y h:i:s A',      // 09/03/2025 04:31:00 PM (with seconds)
                        'Y-m-d H:i:s',        // 2025-03-09 16:31:00
                        'Y/m/d H:i:s',        // 2025/03/09 16:31:00
                        'd/m/Y H:i:s',        // 09/03/2025 16:31:00
                        'd-m-Y H:i:s',        // 09-03-2025 16:31:00
                        'Y-m-d',              // 2025-03-09
                        'd/m/Y',              // 09/03/2025
                        'd-m-Y'               // 09-03-2025
                    ...
                    // If still not parsed, try with strtotime after cleaning the string
                    if ($timestamp === null) {
                        // Clean up the timestamp string (remove extra spaces, normalize)
                        $cleaned_timestamp = preg_replace('/\s+/', ' ', trim($sheet_timestamp));
                        $parsed_timestamp = strtotime($cleaned_timestamp);
                        if ($parsed_timestamp !== false) {
                            $timestamp = date('Y-m-d H:i:s', $parsed_timestamp);
                        }
                    }
