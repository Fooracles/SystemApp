<?php
/**
 * Smart Cron Leave Sync Script with U2/V2 Change Detection
 * Only syncs when U2 (total count) or V2 (blank status count) changes
 * Similar to FMS Task sync logic for efficiency
 */
...
// Log file for smart sync operations
$log_file = __DIR__ . '/../logs/smart_sync.log';

function logSmartSync($message) {
    global $log_file;
    $timestamp = date('Y-m-d H:i:s');
    $log_entry = "[{$timestamp}] SMART: {$message}\n";
...
// Start logging
logSmartSync("=== STARTING SMART SYNC WITH U2/V2 DETECTION ===");

try {
    // Connect to database
    $conn = mysqli_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_NAME);
    if (!$conn) {
        logSmartSync("ERROR: Database connection failed: " . mysqli_connect_error());
        exit(1);
    }
    
    // Set timezone
    mysqli_query($conn, "SET time_zone = '+05:30'");
    logSmartSync("Database connected successfully");
    
    // Include Google Sheets client
    require_once __DIR__ . '/../includes/google_sheets_client.php';
    logSmartSync("Google Sheets client loaded");
    ...
    logSmartSync("Checking U2/V2 cells for changes in sheet: {$sheet_id}, tab: {$tab_name}");
    ...
    if (empty($u2_v2_data) || count($u2_v2_data) < 1) {
        logSmartSync("ERROR: Could not read U2/V2 cells from Google Sheet");
        exit(1);
    }
    ...
    logSmartSync("Current U2 (total count): '{$current_u2}'");
    logSmartSync("Current V2 (blank status count): '{$current_v2}'");
    ...
    if (!$sync_record) {
        ...
        logSmartSync("No sync record found - creating new record and syncing");
        $should_sync = true;
        ...
        logSmartSync("Stored U2: '{$stored_u2}', Stored V2: '{$stored_v2}'");
        ...
        if ($current_u2 !== $stored_u2 || $current_v2 !== $stored_v2) {
            ...
            logSmartSync("SYNC TRIGGERED: {$sync_reason}");
        } else {
            $should_sync = false;
            $sync_reason = "No changes detected - U2 and V2 values unchanged";
            logSmartSync("SKIP SYNC: {$sync_reason}");
        }
    }
    
    // Step 4: Execute sync or skip
    if ($should_sync) {
        logSmartSync("=== EXECUTING SYNC ===");
        logSmartSync("Reason: {$sync_reason}");
        ...
        if (empty($sheet_data)) {
            logSmartSync("WARNING: No data found in sheet after sync trigger");
            exit(0);
        }
        
        $total_rows = count($sheet_data);
        logSmartSync("Google Sheets API returned {$total_rows} data rows");
        ...
        for ($row_index = 0; $row_index < $total_rows; $row_index++) {
            ...
            try {
                        ...
                        if ($affected_rows > 0) {
                            ...
                            if ($exists) {
                                ...
                                if ($old_status !== $status) {
                                    logSmartSync("UPDATED: {$unique_service_no} - {$employee_name} (Status: {$old_status} → {$status})");
                                } else {
                                    logSmartSync("UPDATED: {$unique_service_no} - {$employee_name} (Data refreshed)");
                                }
                            } else {
                                $inserted_count++;
                                logSmartSync("INSERTED: {$unique_service_no} - {$employee_name} (New record)");
                                ...
                                if (empty($status) || $status === 'PENDING' || $status === '') {
                                    ...
                                    logSmartSync("NOTIFICATION: Triggered leave request notification for {$unique_service_no}");
                                }
                            }
                        }
                    }
                ...
                logSmartSync("ERROR: Row " . ($row_index + 3) . " - " . $e->getMessage());
            }
        }
        ...
        // Log final results
        logSmartSync("=== SMART SYNC COMPLETED ===");
        logSmartSync("Total records processed: {$total_rows}");
        logSmartSync("Records synced: {$synced_count}");
        logSmartSync("New records inserted: {$inserted_count}");
        logSmartSync("Existing records updated: {$updated_count}");
        
        if (!empty($errors)) {
            logSmartSync("Errors encountered: " . count($errors));
            foreach ($errors as $error) {
                logSmartSync("ERROR: {$error}");
            }
        }
        ...
        logSmartSync("=== SYNC SKIPPED ===");
        logSmartSync("Reason: {$sync_reason}");
        logSmartSync("Timestamp updated - no data processing needed");
    }
    
    logSmartSync("=== END SMART SYNC ===");
    ...
    logSmartSync("FATAL ERROR: " . $e->getMessage());
    exit(1);
} finally {
    if (isset($conn)) {
        mysqli_close($conn);
